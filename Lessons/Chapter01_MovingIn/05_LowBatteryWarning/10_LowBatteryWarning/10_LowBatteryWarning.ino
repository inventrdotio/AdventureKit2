 ...
 * to give an audible warning instead.
 *
 * Each speaker has a couple connections.  One is an "active buzzer" and that's the simplest
 * way to get a sound since it will emit sound whenever 5 volts is supplied.  Our parts box
 * even has a small active buzzer we can add to our simulation.  Let's start with our
 * last sketch and add a warning before we cut the lights.
 *
 * Define a constant for our buzzer pin and then add another new constant for our new LOW_BATTERY_WARNING and make it 5%
 * higher than our cutoff limit.
 *
 * Now, in our loop() we simply turn the buzzer on when the battery charge is at our warning limit.
 * Let's test that out and see how it works.
 *
 * Second version
 * ==============
 * Oh, dear.  That was NOT better.  I'd almost rather be suddenly plunged into darkness than
 * have that tone blasting in my ears whenever power gets low.  It would keep up that racket
 * until the battery started charging again the next day!
 *
 * I guess this can be a good learning experience.  Code evolves and our early ideas may not
 * always work out the way we like.  In fact, to demonstrate this let's KEEP our first version
 * around to remind us and use more conditional compilation to create version 2 of our low
 * battery warning.  You'll see a define for SKETCH_VERSION below.  If it's set to 1 you'll
 * get the first version of our warning.
 *
 * For version 2, let's just beep a few times to let us know we should finish what we're doing
 * and turn out the lights before they suddenly go out.  
 *
 * We only want to beep when we first
 * drop below the limit so let's add a variable to track the charge level the *previous* time
 * through the loop.  We can set it equal to the starting charge level so we don't get any bad alerts.
 * We'll only beep when the previous level was above the limit and the
 * current value is below the limit.  That is, we'll have it beep when it first crosses that
 * level.
 *
 * At the bottom of the loop we'll copy the current charge level into the variable for the
 * previous charge level for the next time through our loop.
 *
 * Set the SKETCH_VERSION to 2, build and upload so we can try out our new code.  Turn on the light,
 * cover the photoresistor (to simulate night) and let's see if we get a warning *before* the light
 * goes out.
 *
 * Third Version
 * =============
 * Excellent!  That's much less annoying and now we have some warning as our batteries get low so
 * that we can carefully stop what we're doing and get ready for the lights to go out.
 *
 * However, we still have a small issue.  Were you watching the Serial Plotter when it started
 * beeping?  Instead of showing lots of values every second the display froze for the entire time
 * we were beeping.  What's up with that?
 *
 * Oh, of course!  We did our beeping by doing a digitalWrite() followed by a delay().  Those delays
 * are all added to the amount of time used during the loop() execution when we cross below the warning
 * limit.  That's not good.  Let's see if we can get the same beeping *without* using the delay()
 * function since delay pauses the entire program when it is called.
 *
 * What we really want to do is to turn on the buzzer and then, at some future time, turn it back off again.
 * The pin will remain at whatever state we set, so all we have to do is to *toggle* the pin state
 * after some time has passed until we have toggled it on/off the number of times we wish to beep.
 *
 * The Arduino IDE has a way to track how much time your sketch has been running, so if we save the
 * time when we START the beep we can then check each time through the loop until enough time has
 * passed.  The millis() function will return how long the sketch has been executing, in milliseconds
 * (thousandths of a second) just like delay().
 *
 * Once enough time has passed we then toggle the state of the buzzer pin, and each time we
 * turn the buzzer off we'll subtract one from the number of beeps left and save the current
 * millis() value.
 *
 * Set SKETCH_VERSION to 3, upload it to our HERO XL and watch the Serial Plotter when our charge
 * level drops below our warning limit.
 *
 * Version Four
 * ============
 * Things are really coming together now, aren't they?  I think we could stop here, but I have
 * one more idea we can try to put even more polish on our solution.
 *
 * The speakers in our new home can do more than just emit a constant tone.  Instead of just
 * beeping we could use our speakers to play some tones.  In the future we could use different
 * tones to indicate different events, but for now let's modify our sketch to play three tones
 * with descending pitch to indicate our battery level is low.
 *
 * In addition to the active buzzer, our parts cache contains a *passive* buzzer.  How is that
 * different from our active buzzer, you ask?  Well, the active buzzer generates a fixed tone whenever
 * it's connected to 5V.  A passive buzzer, on the other hand, just emits a short "tic" sound when
 * voltage is supplied.  How does that help us?  Well, if we turn the voltage on and off very fast those
 * "tic" sounds run together and can sound like a musical note.
 *
 * We can change how quickly we toggle the voltage to generate different notes.  Lucky for us,
 * the Arduino Language has provided the tone() function that will do that for us, AND it will do this without
 * causing a delay in our loop().  In order to make it easy to make musical notes, you just provide
 * the frequency (in cycles per second, or "Hertz") for the note you desire.  The note will change
 * if you call the tone() function with a new frequency or stop playing if you call noTone().
 *
 * We can save our "tune" in an "array" of variables.  Computer arrays allow us to store multiple
 * values attached to a variable name and use an "index" to indicate which value to use.
 * The index is indicated by adding square brackets ("[]") to the end of the variable name, with the
 * desired index inside the brackets.  You can also use a variable name for the index which allows us
 * to quickly point to any value in an array.
 *
 * The
 * variable name points to the FIRST object in the array, and the index is then added to that address
 * to find additional values.  Because of that, we use an index of zero to point to the first value
 * in an array, an index of one to point to the second value, etc.
 *
 * We can set up a constant array called TONES and initialize it with the frequencies to play in turn
 * to produce a tune.  We'll use the same timing code using millis() to determine when to switch
 * tones or for when it's time to turn off our tune.
 *
 * So, swap out the active buzzer and replace it with the passive buzzer, set SKETCH_VERSION to 4, upload and give that a try.  When the charge level hits our warning level
 * you should now hear a short tune when the charge level reaches our warning level.
 *
 * Next Steps
 * ==========
 * That's enough for now, but here's an idea to try if you have some extra time.
 *
 * We now know when we're about to exhaust our batteries and need to turn off the lights (or have them turned
 * off by our HERO XL!).  But what about in the morning?  Once the sun hits our solar panels the
 * batteries will start to charge again, and we can turn on the lights once we get a bit of charge.
 *
 * Perhaps you could evolve the sketch one more time to play a DIFFERENT tune when the charge level
 * first goes back *above* our warning level?  That will let us know that we can turn the lights
 * back on.
 *
 * Alex Eschenauer
 * David Schmidt
