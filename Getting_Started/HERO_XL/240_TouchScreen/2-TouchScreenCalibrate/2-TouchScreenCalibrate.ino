 ...
 *
 * Contributors:
 * TouchScreenCalibr_native for MCUFRIEND Uno/Mega Display Shields
...
// Read and return one raw point from touch panel, restoring pins back to LCD mode afterwards
...
void setup()
{
  ...
  tft.begin(device_id);  // Start again (restoring pin modes)
  tft.setRotation(TOUCH_ORIENTATION);
  dispx = tft.width();
  dispy = tft.height();
  text_y_center = (dispy / 2) - 6;
  sprintf(buf, "Device ID = 0x%04x", device_id);
  Serial.println(buf);
  if (ret == false) {
